...
object Decompression {
    ...
    def parseBytes(bytes: Array[Byte]): (List[Byte], Map[Char, Int]) = {
    ...
    val readCompressedFile: String => IO[(List[Byte], Map[Char, Int])] = fileName => {
        compressedFileResource(fileName).use(file => IO(parseBytes(file.readAllBytes())))
    }

    def retrieveChars[A <: HuffmanTreeBaseNode](bytes: List[Byte], root: A): IO[String] = root match {
        case node: HuffmanNode => IO(decodeBytes(node, bytes))
